...
variable "gallery_role_assignments" {
  description = <<-EOT
    ...
    (prd: c94a69e3-474e-44d5-8ead-08eb747e2298 from gallery/prd/environment.json).
    Set principal_id to the MSI object ID (lookup build-bp-prd-vdi-mgmt-msi).
  EOT
  type = map(object({
    principal_id         = string
...
variable "pers_host_pools" {
  ...
  type = map(object({
    ...
    rdp_persona          = optional(string, "standard")
