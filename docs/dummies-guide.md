...
| §5–§6 | Naming, tags, provider, state |
| §7 | Stack-by-stack inventory (`_global` → `avd`) |
| §8–§11 | Networks, AVD pools, alerts, labs storage |
| §12–§14 | Wiring map, deploy checklist, one-page cheat sheet |
| §15 | Module **outputs** quick-ref (pairs with §2) |

---

## 1. Purpose and topology

### 1.1 What this programme is doing
...
### 1.3 Architecture — platform overview
...
### 1.4 Architecture — Hub01 (PERS / PRIV / mgmt) traffic
...
### 1.5 Architecture — Hub02 / dual-hub (MSH) traffic
...
### 1.6 Architecture — who builds what (TF vs PowerShell)
...
## 2. Module catalogue — what each brick is for
...
### 2.1 Cross-cutting (no Azure resources)
...
### 2.2 Platform — Virtual WAN and hubs
...
## 3. Azure DevOps, Terraform pipelines, and legacy scripts

### 3.1 Split of responsibility (memorise this)
...
### 3.2 Terraform pipelines in **this** repo
...
### 3.4 Operational pipelines you will still run (unchanged logic)
...
### 3.5 What got **retired** vs **deferred**
...
## 4. Identity, SPNs, and who does what

### 4.1 Azure DevOps service connections (deploy)
...
### 4.3 Terraform-managed platform identities / RBAC
...
| Gallery Packer build MSI | avd | **Custom role GUID** (not Contributor) | gallery RG | empty until tfvars; see §12.2 |
...
### 4.4 Stays PowerShell (do not invent in TF)

Full pipeline ownership is in §3. Short list:
...
## 7. Stack-by-stack — what Terraform creates
...
| Alert rules | Scheduled query + metric + activity log (see §10). **Deployed** in both envs; **`enabled` only when `environment == "prd"`** |
...
### 8.3 PERS / PRIV / mgmt — Hub01 attachment
...
### 8.4 MSH — dual hub + three-rule UDR
...
### 8.5 INT address plan (defaults in `variables.tf`)
...
## 9. Azure Virtual Desktop objects
...
### 9.3 Privileged — 1 pool (default on)
...
- Packer MSI role: assign existing **custom role GUID** only (see §12.2).
...
## 11. Labs storage and Key Vaults

### 11.1 FSLogix — 10 storage accounts
...
### 12.2 Gallery Packer custom roles (tfvars)
...
## 13. Deploy checklist (tfvars you must fill)
...
## 15. Module outputs quick-ref

Companion to §2 (what each module *is*). This table is what each module *returns* for wiring.
